...
В качестве бекенда-сборщика файлов возьмём [nginx](https://nginx.org/ru/) с самописным модулем, который будет осуществлять сборку и разборку файла пользователя
по плану сборки, который поступает на него от бекенда-планировщика. Бекенд планировщик напишем на языке [C++](https://en.wikipedia.org/wiki/C%2B%2B),
а модули которые требуют максимальной безопасности и надёжности на языке [Rust](https://www.rust-lang.org/),
тк он подходит для надёжных высокопроизводительных приложений.
...
Соответственно выходит, что в секнуду в базы RocsDB записывается около 396 Байт * 1500 = 594КБ. Для такой нагрузки хватит и одного среднего сервера с 4ТБ SSD.
...
соотв имеем запас в 3 раза. Также для надёжности будем использовать ещё 2 slave реплики, а репликацию будем осуществлять с помощью open source решение
...
что нам нужно для хранение чанков пользователей. Для такого сервиса в кацестве дисков, на которых будут лежать чанки файлов, целесообразно использовать HDD, тк
...
* Для сервера планировщика запросов требуется 6 SSD по 4ГБ в RAID10 для хранения (это 12ГБ + ускорение записи в 2 раза, чтения в 4 раза),
...
* Для серверов nginx-сборщиков потребуется мощный процессор и много оперативной памяти, диск им не нужен, тк HTML, CSS, JS они могут отдавать с серверов-хранилищ
и кешировать его.
...
* [стенд от WD](https://www.westerndigital.com/products/data-center-platforms/ultrastar-serv60-8-hybrid-server)
...
Многое по этому пункту уже было сказано в предыдущих пунктах, но некоторые вещи можно ещё добавить, а именно:

* Использование [bonding](https://habr.com/ru/post/58218/) для объединения сетевых интерфейсов в Linux. Это даст большую пропускную способность, а также позволит
повысить отказоустойчивость в случае, если один из каналов станет недоступен
